...
from mmdet.models.backbones.vit_mae import ViTMAE
from mmdet.models.necks.upsample import UpSampler
...
data_preprocessor = dict(
    ...
    bgr_to_rgb=True,
    pad_size_divisor=32)
strides = [2, 4, 8, 16, 32, 64] # 400 / 200, 400 / 100, 400 / 50, 400 / 25, 400 / 13, 400 / 7 but we only use 5 feature levels?
...
model = dict(
    ...
    backbone=dict(
        ...
        patch_size=16,
        embed_dim=1024,
        depth=16, # should be 24 on V100's / HPC
        num_heads=16,
        mlp_ratio=4,
    ...
    neck=dict(
        type=UpSampler, #input size at 400px by 400px is [25, 25, 1024] output is [200, 200, 128]
        ),
    query_head=dict(
        type='CoDINOHead',
        num_query=900,
